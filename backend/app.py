...
import json
import time
from config import model_config, AVAILABLE_MODELS
...
def detect_document_domain(text_sample: str) -> str:
    ...
    medical_terms = [
        'patient', 'diagnosis', 'treatment', 'medication', 'doctor', 'physician', 'nurse',
        'hospital', 'clinic', 'medical', 'surgery', 'procedure', 'symptoms', 'condition',
        'prescription', 'dosage', 'therapy', 'examination', 'vital signs', 'blood pressure',
        'heart rate', 'temperature', 'respiratory', 'cardiovascular', 'neurological',
        'pathology', 'radiology', 'laboratory', 'discharge', 'admission', 'anesthesia',
        'post-operative', 'pre-operative', 'clinical', 'healthcare', 'medical record'
    ]
    
    # Legal terminology indicators
    legal_terms = [
        'contract', 'agreement', 'clause', 'party', 'parties', 'plaintiff', 'defendant',
        'court', 'judge', 'attorney', 'lawyer', 'legal', 'law', 'statute', 'regulation',
        'compliance', 'liability', 'damages', 'settlement', 'litigation', 'jurisdiction',
        'whereas', 'therefore', 'hereby', 'witnesseth', 'consideration', 'covenant',
        'indemnify', 'breach', 'termination', 'arbitration', 'governing law', 'venue'
    ]
...
def create_embeddings_for_document(pages_data: List[Dict]) -> bool:
    ...
    try:
        ...
        if documents:
            ...
            print(f"Created embeddings for {len(documents)} chunks across {len(pages_data)} pages")
            return True
...
def semantic_search(question: str, top_k: int = 5) -> List[Dict]:
    ...
    try:
        ...
        for i, result in enumerate(search_results):
            ...
            print(f"  {i+1}. Page {result['page']}, {chunk_type}, similarity: {similarity:.3f}")
...
def answer_question_with_openai(question: str, context_text: str, page_sources: List[int]) -> str:
    ...
    try:
        ...
        # Enhanced prompt
        prompt = f"""You are an expert document analyst. Based on the following document content from {page_info}, please answer the question with precision and accuracy.
...
def answer_question_with_gemini(question: str, context_text: str, page_sources: List[int]) -> str:
...
def answer_question_with_embeddings(question: str, pages_data: List[Dict]) -> Tuple[str, List[Dict]]:
    ...
    # Combine relevant text chunks for context with clear page structure
    context_parts = []
    ...
    context_text = "\n\n".join(context_parts)
    page_sources = [chunk["page"] for chunk in relevant_chunks]
    ...
    for page_num in range(len(doc)):
        ...
        if len(page_text.strip()) < 50:
            ...
            if len(simple_text.strip()) > len(page_text.strip()):
                ...
                text_blocks = []
                line_height = page_rect.height / max(len([l for l in lines if l.strip()]), 1)
                y_offset = 50
                
                for line in lines:
                    line = line.strip()
                    if line:
                        bbox = [50, y_offset, page_rect.width - 50, y_offset + line_height]
                        text_blocks.append({
                            "text": line,
                            "bbox": bbox,
                            "page": page_num + 1,
...
def ask_question():
    ...
    try:
        ...
        if embedding_model and collection:
            ...
            answer, _ = answer_question_with_embeddings(question, current_pdf_pages)
        else:
